...
hist(death_ages_with_cv, col='gray', border='darkgray', breaks=seq(0,90,1))
hist(death_ages_with_cv_trunc, col='red', border='darkgray', breaks=seq(0,90,1), add=T)
curve(n*alpha_actual * exp(beta_actual*x) * exp((-alpha_actual/beta_actual) * (exp(beta_actual*x)-1)), 
      from=0, to=85, add=TRUE, col = 'darkblue', lty=2, lwd=2)
```
...
# initialize parameters
init_gomp_trunc_mv <- function () {list(beta = 0.07, mode = 65, b = c(0,0))} 
