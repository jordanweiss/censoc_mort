...
dt <- fread("../data/censoc_numident_demo_v1.csv")
...
Clean up estimates 
```{r}
test <- summary(numident_with_covariate_fit)

stan_educ_estimates <- test$summary %>% 
  as.data.frame() %>% 
  mutate(across(where(is.double), ~round(x = .x, digits = 3))) %>% 
  select(estimate = mean, lower = "2.5%", upper = "97.5%") %>% 
  rownames_to_column("parameter") %>% 
  rename_at(vars(-parameter),function(x) paste0(x,"_stan")) %>% 
  filter(parameter != "lp__") %>% 
  mutate(parameter = case_when(
    parameter == "mode[1]" ~ "coh1915",
    parameter == "mode[2]" ~ "coh1916",
    parameter == "mode[3]" ~ "coh1917",
    parameter == "mode[4]" ~ "coh1918",
    parameter == "mode[5]" ~ "coh1919",
    TRUE ~ parameter
  ))
```


Compare with education parameter estimates from MLE. This requires running a separate script! 

```{r}
mle_educ_estimates <- mle %>% 
  select(estimate = hat, lower = "hat.lower", upper = "hat.upper") %>% 
  rownames_to_column("parameter") %>% 
  rename_at(vars(-parameter),function(x) paste0(x,"_mle"))

education_comparison <- stan_educ_estimates %>% 
  inner_join(mle_educ_estimates)
