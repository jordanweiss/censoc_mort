...
dt = fread("data/analysis_file_restricted.csv")  ## demo Numident
...
## compare to OLS
m <- lm(y ~ covar + as.factor(cohort), data = A)
## -------------- edu -----------
...
get.trunc.mean.gomp <- function(alpha, beta, l, u)
{
  ...
  sum(lx)
  dx = -diff(lx)
  s <- x %in% l:u ## approximate trunccation for these cohorts
  sum((dx*x)[s])/sum(dx[s])
}

## cohort of 1920 (u = 86, u = 85)
truncated_untreated <- (get.trunc.mean.gomp(my.alpha, .1, 68, 85))

truncated_treated <- (get.trunc.mean.gomp(my.alpha*(.86), .1, 68, 85))

truncated_treated - truncated_untreated
## very close to OLS
